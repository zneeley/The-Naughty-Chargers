...
if(isset($_FILES['image'])){
    ...
    $fileDir = "uploadContent/userProfilePics/".$fileNameNew;
    move_uploaded_file($fileTmp,"uploadContent/userProfilePics/".$fileNameNew);
}

// Store information into database and upload image
if(isset($_FILES['image'])) {
    if (empty($imgExt_err) && empty($imgSize_err)) {
        // Prepare an insert statement
        $sql = "UPDATE users SET profileImage = ? WHERE userID = ?";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ss", $param_profileImage, $param_userID);
